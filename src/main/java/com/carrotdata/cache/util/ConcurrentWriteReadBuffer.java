...
/**
 * 
 * Concurrent (circular) memory buffer for writes and reads. Every write
 * has a maximum size known in advance - page size.
 * It requires maximum number of concurrent threads 
 * to be known in advance as well. Writes are lock - free,
 * reads are lock - free and wait free. To implement
 * wait free and lock free reads we set size of 
 * an internal memory buffer to 2 * max_threads * 2 * page_size
 * and if distance between read slot and current write slot exceeds
 * maximum number of threads we fail the read. The reason is two-fold:
 * this gives us guarantee that read cell won't overwritten by writes 
 * during read operation and the requested data should have been evacuated
 * already by thus time, so a caller can call other storage for a data
 * Limitations: page size <=32K (can be increased by a few line code changes), 
 * maximum number of threads <= 16K ()
 * 
 */
...
public class ConcurrentWriteReadBuffer {
   ...
   * @return index of a record (4 bytes - write run number, 
   * 2 bytes slot number, 2 bytes offset in a slot
   */
  public long append(long keyPtr, int keySize, long valuePtr, int valueSize, long expire) {
    int requiredSize = keySize + valueSize + Utils.sizeUVInt(keySize) + 
        Utils.sizeUVInt(valueSize) + Utils.SIZEOF_LONG;
    ...
    if (off + requiredSize >= this.pageSize) {
      ...
      // The last one is slotWriteOffset - it cancels all other threads busy loops at the beginning of 
  ...
  public int read(long index, long buffer, int bufferSize) {
    ...
    if (curWriteRun >= writeRun) {
      long dist = (long) (curWriteRun - writeRun) * this.numSlots + this.writeSlotNumber - slotNumber;
  ...
  /**
   * Return address of a k-v record or -1. This call
   * avoid double memory copy and allows to copy data directly from
   * this cache storage to some output buffer
