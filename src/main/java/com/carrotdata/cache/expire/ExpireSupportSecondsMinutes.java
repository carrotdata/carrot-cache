 ...
 * field. There are additional 2 2 bytes fields in the Meta section of each index block
 */
public class ExpireSupportSecondsMinutes extends AbstractExpireSupport {
  ...
  private final long getExpire0(long ibesPtr, long expireFieldPtr) {
    ...
    if (isSeconds) {
      long start =
          this.epochStartTime + getSecondsEpochCounterValue(ibesPtr) * EPOCH_DURATION_SEC_SM_TYPE;

      return start + value * 1000;
    } else { // minutes
      value = low15(value);
      long start =
          this.epochStartTime + getMinutesEpochCounterValue(ibesPtr) * EPOCH_DURATION_MIN_SM_TYPE;
