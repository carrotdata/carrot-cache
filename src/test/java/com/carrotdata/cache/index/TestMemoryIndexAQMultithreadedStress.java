 ...
 */
public abstract class TestMemoryIndexAQMultithreadedStress
    extends TestMemoryIndexMultithreadedBase {
  /** Logger */
  private static final Logger LOG =
      LoggerFactory.getLogger(TestMemoryIndexAQMultithreadedStress.class);
  ...
  private int loadIndexBytes() {
    ...
    LOG.info(Thread.currentThread().getName() + " loaded " + numRecords + " RPS="
  ...
  private int verifyIndexBytes() {
    ...
    long end = System.currentTimeMillis();
    LOG.info(Thread.currentThread().getName() + " verified " + numRecords + " RPS="
        + ((long) numRecords) * 1000 / (end - start));
  ...
  private int loadIndexBytesNot() {
    ...
    long end = System.currentTimeMillis();
    LOG.info(Thread.currentThread().getName() + " loaded NOT " + numRecords + " RPS="
        + ((long) numRecords) * 1000 / (end - start));
    return failed;
  }
