...
public class IOCompressionTestBase extends IOTestBase {
  ...
  protected void initTestForSegment(boolean randomData, boolean dictionaryEnabled)
  ...
  protected void initTestForEngine(boolean randomData, boolean dictionaryEnabled)
      throws URISyntaxException, IOException {
    initTest(randomData, dictionaryEnabled);
  ...
  protected void initTest(boolean randomData, boolean dictionaryEnabled)
  ...
  protected void verifyBytes(int num) {
    ...
    for (int i = 0; i < num; i++) {
      ...
      assertTrue(Utils.compareTo(value, 0, value.length, buf, off + kSizeSize + vSizeSize + kSize,
        vSize) == 0);
  ...
  protected void verifyMemory(int num) {
    ...
    for (int i = 0; i < num; i++) {
      ...
      assertTrue(
        Utils.compareTo(buf, off + kSizeSize + vSizeSize + kSize, vSize, mValue, vSize) == 0);
      off += kSize + vSize + kSizeSize + vSizeSize;
    }
  }
}
