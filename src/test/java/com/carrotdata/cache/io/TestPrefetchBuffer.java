...
public class TestPrefetchBuffer extends IOTestBase {
  ...
  public void testPrefetchBufferWithBaseWriter() throws IOException {
    ...
    WriteBatches wb = new WriteBatches(writer);
    Mockito.when(engine.getWriteBatches()).thenReturn(wb);
    segment.setDataWriterAndEngine(new BaseDataWriter(), engine); 
    int n = loadBytes();
    WriteBatch batch = wb.writeBatches().iterator().next();
    n -= batch.size(); // Batch data won't be saved
