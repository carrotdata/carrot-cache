...
public abstract class TestIOMultithreadedBase {
   ...
   * @return true on success, false - otherwise
   */
  protected abstract boolean put(long keyPtr, int keySize, long valuePtr, int valueSize,
   ...
   */
  protected abstract long get(byte[] key, int off, int len, byte[] buffer, int bufferOfset)
      throws IOException;
  ...
  private void testLoadReadBytes() throws IOException {
    /* DEBUG */ LOG.info(Thread.currentThread().getName() + ": testLoadReadBytes");
    prepareDataBytes();
  ...
  private void testLoadReadMemory() throws IOException {
    /* DEBUG */ LOG.info(Thread.currentThread().getName() + ": testLoadReadMemory");
    prepareDataMemory();
  ...
  private void testLoadReadBytesWithDeletes() throws IOException {
    /* DEBUG */ LOG.info(Thread.currentThread().getName() + ": testLoadReadBytesWithDeletes");
    prepareDataBytes();
    int loaded = loadBytes();
    /* DEBUG */ LOG.info(Thread.currentThread().getName() + ": loaded=" + loaded);
  ...
  private void testLoadReadMemoryWithDeletes() throws IOException {
    /* DEBUG */ LOG.info(Thread.currentThread().getName() + ": testLoadReadMemoryWithDeletes");
    prepareDataMemory();
    int loaded = loadMemory();
    /* DEBUG */ LOG.info(Thread.currentThread().getName() + ": loaded=" + loaded);
